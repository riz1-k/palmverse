...
const createOption = (value: any, options: any) => {
  ...
  if (!Array.isArray(value) && (value || value === 0)) {
    const valueObj = options.reduce(
      (obj, option) => (option.value === value ? option : obj),
      {}
    );
    return valueObj;
  }
...
const Select: React.FC<TypeSelectProps> = ({
  ...
  const handleChange = (valueObj: { value: any }) => {
    ...
    if (valueObj !== null) {
      value = Array.isArray(valueObj)
        ? valueObj.map((value) => value.value)
        : valueObj.value;
    }
  ...
  if (validity === false && isTouched)
    wrapperClasses.push('select--invalid');
 ...
 */
function areEqual(
  prevProps: TypeSelectProps,
  nextProps: TypeSelectProps
) {
